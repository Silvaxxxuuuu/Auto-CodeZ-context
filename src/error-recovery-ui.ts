...
function installStyles(): void {
  ...
  style.textContent = `
    ...
    @media(max-width:700px){.ac-recovery-panel{align-items:flex-start;flex-direction:column}.ac-recovery-actions{width:100%}.ac-recovery-action{flex:1}}
    @media(prefers-reduced-motion:reduce){.ac-recovery-action,.ac-ui-toast{transition:none;animation:none}}
  `;
...
function openApiKeys(): void {
  ...
  const configure = document.querySelector<HTMLButtonElement>('[data-action="ai-settings"]');
  configure?.click();
}
...
function recoveryCopy(kind: ReturnType<typeof classify>): { title: string; description: string; action: string; primary: boolean } {
  switch (kind) {
    case 'authentication':
      return { title: 'Verifique sua API key', description: 'A chave foi recusada. Abra as credenciais para trocar ou adicionar outra chave.', action: 'Abrir API Keys', primary: true };
    case 'billing':
      return { title: 'Conta sem créditos disponíveis', description: 'Sua chave continua salva. Você pode adicionar outra chave ou revisar as credenciais.', action: 'Abrir API Keys', primary: true };
    case 'quota':
      return { title: 'Cota do modelo atingida', description: 'Troque o modelo ou selecione outra API key nas configurações deste chat.', action: 'Configurar IA', primary: true };
    case 'rate_limit':
      return { title: 'Limite de requisições atingido', description: 'Aguarde alguns instantes e envie a mensagem novamente.', action: 'Tentar novamente', primary: false };
    case 'network':
      return { title: 'Falha de conexão', description: 'Verifique sua conexão com a internet e tente novamente.', action: 'Tentar novamente', primary: false };
    case 'server':
      return { title: 'Falha temporária do provider', description: 'O serviço remoto apresentou um erro. Tente novamente em instantes.', action: 'Tentar novamente', primary: false };
    default:
      return { title: 'A solicitação falhou', description: 'Revise as configurações da IA ou tente enviar a mensagem novamente.', action: 'Configurar IA', primary: true };
  }
}
...
function retryLastMessage(): void {
  ...
  if (!last) {
    createToast('Não há uma mensagem anterior para reenviar.');
    return;
  }
...
function syncRecovery(): void {
  ...
  panel.querySelector<HTMLButtonElement>('[data-recovery-action]')?.addEventListener('click', () => {
    if (kind === 'authentication' || kind === 'billing') {
      openApiKeys();
      return;
    }
    if (kind === 'quota') {
      document.querySelector<HTMLButtonElement>('.provider-chip')?.click();
      return;
    }
    if (kind === 'rate_limit' || kind === 'network' || kind === 'server') {
      retryLastMessage();
      return;
    }
