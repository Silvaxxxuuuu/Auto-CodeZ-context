...
export function fingerprintRequest(request: AIRequest): string {
  const normalized = stableValue({ providerId: request.providerId, model: request.model, messages: request.messages, intelligence: request.intelligence, projectContext: request.projectContext, toolsEnabled: request.toolsEnabled, tools: request.tools });
...
export class ProviderRequestJournal {
  ...
  async init(): Promise<void> {
    ...
    for (const entry of state.entries) if (entry?.requestId && entry.fingerprint && entry.providerId && entry.model) this.entries.set(entry.requestId, { ...entry });
  }

  async begin(request: AIRequest): Promise<{ requestId: string; cachedResponse?: AIResponse }> {
    const fingerprint = fingerprintRequest(request);
    const existing = [...this.entries.values()].filter((entry) => entry.fingerprint === fingerprint).sort((a, b) => b.updatedAt - a.updatedAt)[0];
  ...
  async complete(requestId: string, response: AIResponse): Promise<void> { const entry = this.require(requestId); entry.status = 'completed'; entry.response = response; delete entry.error; entry.updatedAt = Date.now(); await this.persist(); }
  ...
  list(): ProviderRequestJournalEntry[] { return [...this.entries.values()].map((entry) => ({ ...entry, response: entry.response ? { ...entry.response, toolCalls: entry.response.toolCalls ? [...entry.response.toolCalls] : undefined } : undefined })); }
