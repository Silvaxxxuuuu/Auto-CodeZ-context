...
export class OpenAIAdapter implements AIProviderAdapter {
  ...
  async listModels(config: AIProviderConfig): Promise<AIModel[]> {
    ...
    if (!response.ok) {
      ...
      throw createProviderRequestError('OpenAI', 'list-models', response.status, providerErrorMessage(data, `OpenAI models request failed: ${response.status}`));
    }
  ...
  async *stream(config: AIProviderConfig, request: AIRequest): AsyncGenerator<AIStreamEvent> {
    ...
    if (!response.ok) {
      ...
      throw createProviderRequestError('OpenAI', 'stream', response.status, data.error?.message || `OpenAI streaming request failed: ${response.status}`);
    }
