...
function toGoogleSchema(value: unknown): GoogleSchema | unknown {
  ...
  for (const [key, item] of Object.entries(source)) {
    ...
    if (key === 'properties' && item && typeof item === 'object' && !Array.isArray(item)) { schema.properties = Object.fromEntries(Object.entries(item as Record<string, unknown>).map(([name, property]) => [name, toGoogleSchema(property)])); continue; }
    schema[key] = toGoogleSchema(item);
  }
  return schema;
}
function buildTools(request: AIRequest): Array<Record<string, unknown>> | undefined { if (!request.toolsEnabled || !request.tools?.length) return undefined; return [{ functionDeclarations: request.tools.map((tool) => ({ name: tool.name, description: tool.description, parameters: toGoogleSchema(tool.parameters) })) }]; }
function buildContents(messages: AIMessage[]): Array<Record<string, unknown>> {
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    if (message.role === 'tool') {
      ...
      while (index < messages.length && messages[index].role === 'tool') { const toolMessage = messages[index]; if (toolMessage.toolName) parts.push({ functionResponse: { name: toolMessage.toolName, response: { result: toolMessage.content }, ...(toolMessage.toolCallId ? { id: toolMessage.toolCallId } : {}) } }); index += 1; }
      index -= 1; if (parts.length) contents.push({ role: 'user', parts }); continue;
    }
    if (message.role === 'assistant' && message.toolCalls?.length) { const parts: GooglePart[] = []; if (message.content) parts.push({ text: message.content }); for (const call of message.toolCalls) parts.push({ functionCall: { name: call.name, args: call.input, id: call.id }, ...(call.providerData?.thoughtSignature ? { thoughtSignature: String(call.providerData.thoughtSignature) } : {}) }); contents.push({ role: 'model', parts }); continue; }
    contents.push({ role: message.role === 'assistant' ? 'model' : 'user', parts: [{ text: message.content }] });
  }
  return contents;
}
function parseToolCalls(parts: GooglePart[]): AIToolCall[] { return parts.flatMap((part) => { if (!part.functionCall?.name) return []; return [{ id: part.functionCall.id || `google_${part.functionCall.name}_${Math.random().toString(36).slice(2)}`, name: part.functionCall.name as AIToolCall['name'], input: part.functionCall.args || {}, providerData: part.thoughtSignature ? { thoughtSignature: part.thoughtSignature } : undefined }]; }); }
function providerErrorMessage(value: unknown, fallback: string): string { if (!value || typeof value !== 'object') return fallback; const error = (value as { error?: unknown }).error; if (error && typeof error === 'object' && typeof (error as { message?: unknown }).message === 'string') return (error as { message: string }).message; if (typeof error === 'string' && error.trim()) return error; return fallback; }
...
export class GoogleAdapter implements AIProviderAdapter {
  ...
  async listModels(config: AIProviderConfig): Promise<AIModel[]> {
    ...
    if (!response.ok) { const data = await response.json().catch(() => ({})); throw createProviderRequestError('Google AI', 'list-models', response.status, providerErrorMessage(data, `Google models request failed: ${response.status}`)); }
    const data = (await response.json()) as { models?: Array<{ name: string; displayName?: string; supportedGenerationMethods?: string[] }> };
    return (data.models || []).filter((model) => model.supportedGenerationMethods?.includes('generateContent')).map((model): AIModel => { const id = model.name.replace(/^models\//, ''); const thinking = isThinkingModel(id); return { id, name: model.displayName || id, providerId: this.id, capabilities: thinking ? ['text', 'vision', 'streaming', 'tools', 'reasoning'] : ['text', 'vision', 'streaming', 'tools'], reasoningLevels: thinking ? ['low', 'normal', 'high', 'maximum'] : ['normal'] }; });
  }
  private buildBody(request: AIRequest): Record<string, unknown> {
    ...
    if (isGemini3Model(request.model)) body.generationConfig = { thinkingConfig: { thinkingLevel: thinkingLevel(request.intelligence) } }; else if (isGemini25Model(request.model)) body.generationConfig = { thinkingConfig: { thinkingBudget: thinkingBudget(request.intelligence) } };
    const tools = buildTools(request); if (tools) body.tools = tools; return body;
  }
  ...
  async *stream(config: AIProviderConfig, request: AIRequest): AsyncGenerator<AIStreamEvent> {
    ...
    let content = ''; let usage: AIResponse['usage']; const toolCalls: AIToolCall[] = []; let terminal = false; yield { type: 'start' };
    for await (const raw of parseSSE(response)) {
      ...
      for (const part of parts) { if (part.thought) continue; if (part.text) { content += part.text; yield { type: 'delta', text: part.text }; } if (part.functionCall) { const parsed = parseToolCalls([part]); for (const call of parsed) { toolCalls.push(call); yield { type: 'tool_call', toolCall: call }; } } }
