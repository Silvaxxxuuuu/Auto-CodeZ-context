...
export class GoogleAdapter implements AIProviderAdapter {
  ...
  async listModels(config: AIProviderConfig): Promise<AIModel[]> {
    ...
    if (!response.ok) {
      ...
      throw new Error(providerErrorMessage(data, `Google models request failed: ${response.status}`));
    }
  ...
  async *stream(config: AIProviderConfig, request: AIRequest): AsyncGenerator<AIStreamEvent> {
    ...
    if (!response.ok) { const data = await response.json().catch(() => ({})) as GoogleChunk; throw new Error(data.error?.message || `Google AI streaming request failed: ${response.status}`); }
    let content = '';
