...
function buildMessages(messages: AIMessage[]): Array<Record<string, unknown>> {
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    if (message.role === 'tool') {
      ...
      while (index < messages.length && messages[index].role === 'tool') { const toolMessage = messages[index]; if (toolMessage.toolCallId) content.push({ type: 'tool_result', tool_use_id: toolMessage.toolCallId, content: toolMessage.content }); index += 1; }
      index -= 1; if (content.length) result.push({ role: 'user', content }); continue;
    }
    if (message.role === 'assistant' && message.toolCalls?.length) { const content: Array<Record<string, unknown>> = []; if (message.content) content.push({ type: 'text', text: message.content }); for (const call of message.toolCalls) content.push({ type: 'tool_use', id: call.id, name: call.name, input: call.input }); result.push({ role: 'assistant', content }); continue; }
    result.push({ role: message.role, content: message.content });
  }
  return result;
}
function buildTools(request: AIRequest): Array<Record<string, unknown>> | undefined { if (!request.toolsEnabled || !request.tools?.length) return undefined; return request.tools.map((tool) => ({ name: tool.name, description: tool.description, input_schema: tool.parameters })); }
...
function parseToolBlocks(content: Array<{ type?: string; id?: string; name?: string; input?: unknown }>): AIToolCall[] { return content.flatMap((item) => { if (item.type !== 'tool_use' || !item.id || !item.name) return []; const input = item.input && typeof item.input === 'object' && !Array.isArray(item.input) ? item.input as Record<string, unknown> : {}; return [{ id: item.id, name: item.name as AIToolCall['name'], input }]; }); }
...
export class AnthropicAdapter implements AIProviderAdapter {
  ...
  async listModels(config: AIProviderConfig): Promise<AIModel[]> {
    ...
    if (!response.ok) { const data = await response.json().catch(() => ({})); throw createProviderRequestError('Anthropic', 'list-models', response.status, providerErrorMessage(data, `Anthropic models request failed: ${response.status}`)); }
  ...
  async send(config: AIProviderConfig, request: AIRequest): Promise<AIResponse> {
    ...
    if (!response.ok) throw createProviderRequestError('Anthropic', 'send', response.status, data.error?.message || `Anthropic request failed: ${response.status}`);
  ...
  async *stream(config: AIProviderConfig, request: AIRequest): AsyncGenerator<AIStreamEvent> {
    ...
    if (!response.ok) { const data = await response.json().catch(() => ({})) as { error?: { message?: string } }; throw createProviderRequestError('Anthropic', 'stream', response.status, data.error?.message || `Anthropic streaming request failed: ${response.status}`); }
