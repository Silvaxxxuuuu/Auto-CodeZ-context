...
export type ToolName =
  | 'read_file' | 'write_file' | 'create_file' | 'delete_file' | 'rename_file' | 'search_files' | 'run_command'
  | 'git_status' | 'git_diff' | 'git_log' | 'git_branches' | 'git_create_branch' | 'git_checkout' | 'git_stage' | 'git_stage_all' | 'git_commit';
...
export interface AIResponse { content: string; model: string; providerId: ProviderId; usage?: { inputTokens?: number; outputTokens?: number; totalTokens?: number }; toolCalls?: AIToolCall[]; }
export interface AIStreamEvent { type: 'start' | 'delta' | 'activity' | 'tool_call' | 'usage' | 'complete' | 'approval_required' | 'error'; text?: string; activity?: ActivityEvent; toolCall?: AIToolCall; usage?: AIResponse['usage']; response?: AIResponse; pendingApprovalIds?: string[]; error?: string; }
