type StreamEvent = { type?: string; chatId?: string; runId?: string; activity?: { message?: string; status?: string; type?: string } };
type ExecutionSnapshot = { chatId: string; runId: string; state: 'idle' | 'running' | 'waiting_approval' | 'completed' | 'failed' | 'interrupted'; startedAt: number; updatedAt: number };
type Bridge = { onStreamEvent: (listener: (event: StreamEvent) => void) => () => void; listExecutions?: () => Promise<unknown> };
type ThinkingState = { active: boolean; waitingApproval: boolean; runId?: string; startedAt: number; accumulatedMs: number; pausedAt: number; token: number };
...
function elapsed(state: ThinkingState, now = Date.now()): number { if (!state.startedAt || (state.waitingApproval && state.pausedAt)) return state.accumulatedMs; return state.accumulatedMs + Math.max(0, now - state.startedAt); }
function seconds(ms: number): string { const value = Math.max(1, Math.round(ms / 1000)); return `${value} ${value === 1 ? 'segundo' : 'segundos'}`; }

function installStyle(): void {
  if (document.getElementById('auto-codez-thinking-ui')) return;
  const style = document.createElement('style');
  style.id = 'auto-codez-thinking-ui';
  style.textContent = `.ac-thinking-status{max-width:800px;margin:0 auto 7px;color:#7b8491;font-size:11px;line-height:18px}.ac-thinking-dots{display:inline-flex;min-width:17px;margin-left:4px;letter-spacing:2px}.ac-thinking-dots span{opacity:.25;animation:ac-thinking-dot 1.05s infinite}.ac-thinking-dots span:nth-child(2){animation-delay:.15s}.ac-thinking-dots span:nth-child(3){animation-delay:.3s}.ac-thought-time{margin:0 0 6px;color:#737d8a;font-size:10px;line-height:16px}@keyframes ac-thinking-dot{0%,100%{opacity:.25;transform:translateY(0)}35%{opacity:1;transform:translateY(-1px)}70%{opacity:.25;transform:translateY(0)}}@media(prefers-reduced-motion:reduce){.ac-thinking-dots span{animation:none;opacity:.7}}`;
  document.head.appendChild(style);
}

function removeLiveStatus(): void { document.querySelectorAll('.ac-thinking-status').forEach((node) => node.remove()); }
function renderLiveStatus(): void {
  const chatId = activeChatId; if (!chatId) return;
  const state = stateFor(chatId); removeLiveStatus(); if (!state.active) return;
...
function insertThoughtTime(chatId: string, token: number, durationMs: number): void {
  ...
  const messages = document.querySelector<HTMLElement>('#messages'); if (!messages) return;
  const state = stateFor(chatId); if (state.active || state.token !== token) return;
...
function hydrate(): void {
  ...
  void bridge.listExecutions().then((value) => {
    ...
    const active = value.filter((item): item is ExecutionSnapshot => { if (!item || typeof item !== 'object') return false; const snapshot = item as Partial<ExecutionSnapshot>; return typeof snapshot.chatId === 'string' && typeof snapshot.runId === 'string' && typeof snapshot.state === 'string' && typeof snapshot.startedAt === 'number'; });
