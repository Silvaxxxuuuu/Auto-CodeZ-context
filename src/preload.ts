...
contextBridge.exposeInMainWorld('autoCodez', {
  ...
  completeAccountPasskey: (input: unknown) => invoke('account-auth:complete-passkey', requireObject(input, 'Credencial Passkey')),
  ...
  onOperationalLedgerEvent: (listener: (event: unknown) => void) => {
    ...
    return () => ipcRenderer.removeListener('operational-ledger:event', handler);
  },
