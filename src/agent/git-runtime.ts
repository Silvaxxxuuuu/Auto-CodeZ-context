...
export class GitRuntime {
  ...
  async stage(projectId: string, paths: string[]): Promise<GitOperationSummary> { if (!Array.isArray(paths) || paths.length === 0) throw new Error('Selecione ao menos um arquivo para adicionar ao commit.'); const safePaths = paths.map(requireSafePath); const output = await this.git(projectId, ['add', '--all', '--', ...safePaths]); const status = await this.status(projectId); return { operation: 'stage', output: output.trim(), branch: status.branch }; }
  async stageAll(projectId: string): Promise<GitOperationSummary> { const output = await this.git(projectId, ['add', '--all']); const status = await this.status(projectId); return { operation: 'stage_all', output: output.trim(), branch: status.branch }; }
  async commit(projectId: string, message: string): Promise<GitOperationSummary> { const commitMessage = requireCommitMessage(message); const output = await this.git(projectId, ['commit', '-m', commitMessage]); const status = await this.status(projectId); return { operation: 'commit', output: output.trim(), branch: status.branch }; }
}
