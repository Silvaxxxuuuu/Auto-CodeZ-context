...
export class GitRuntime {
  ...
  async stage(projectId: string, paths: string[]): Promise<GitOperationSummary> { if (!Array.isArray(paths) || paths.length === 0) throw new Error('Selecione ao menos um arquivo para adicionar ao commit.'); const safePaths = paths.map(requireSafePath); const output = await this.git(projectId, ['add', '--', ...safePaths]); const status = await this.status(projectId); return { operation: 'stage', output: output.trim(), branch: status.branch }; }
