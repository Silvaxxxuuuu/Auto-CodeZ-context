...
export class AgentRuntime {
  ...
  async init(): Promise<void> {
    ...
    for (const run of stored.runs) {
      ...
      if (validIds.length) for (const id of validIds) this.pendingRuns.set(id, pending);
      else this.recoverableRuns.set(runId, pending);
    }
    await this.persist();
  }
  ...
  private async runLoop(run: PendingRun): Promise<AgentRunResult> {
    while (run.toolRounds < MAX_TOOL_ROUNDS) {
      ...
      if (!response.toolCalls?.length) {
        ...
        this.activity.emit({ runId: run.runId, type: 'complete', message: 'Execução concluída.', status: 'success' });
      ...
      run.toolRounds += 1;
      this.activity.emit({ runId: run.runId, type: 'tool', message: `Executando ${response.toolCalls.length} ferramenta(s).`, status: 'running' });
      ...
      if (pendingApprovalIds.length) {
        ...
        this.activity.emit({ runId: run.runId, type: 'action', message: 'O agente aguarda aprovação antes de continuar.', status: 'pending' });
        await this.persist();
        return { chatId: run.chat.id, response, toolRounds: run.toolRounds, pendingApprovalIds, messages: [...run.workingChat.messages] };
      }
      this.activity.emit({ runId: run.runId, type: 'complete', message: `Ciclo de ferramentas ${run.toolRounds} concluído.`, status: 'success' });
      await this.persist();
    }
    this.recoverableRuns.delete(run.runId);
    await this.persist();
  ...
  private async runStreamLoop(run: PendingRun): Promise<AgentRunResult> {
    ...
    while (run.toolRounds < MAX_TOOL_ROUNDS) {
      ...
      if (!response.toolCalls?.length) {
        run.workingChat.messages.push({ role: 'assistant', content: response.content, createdAt: Date.now() });
        const completion: AIStreamEvent = { type: 'activity', activity: { runId: run.runId, type: 'complete', message: 'Execução concluída.', status: 'success' } };
      ...
      if (!run.workingChat.projectId) throw new Error('Uma ferramenta foi solicitada sem um projeto ativo.');
      run.toolRounds += 1;
      const roundActivity: AIStreamEvent = { type: 'activity', activity: { runId: run.runId, type: 'tool', message: `Executando ${response.toolCalls.length} ferramenta(s).`, status: 'running' } };
      ...
      if (pendingApprovalIds.length) {
        ...
        this.recoverableRuns.delete(run.runId);
        for (const approvalId of pendingApprovalIds) this.pendingRuns.set(approvalId, run);
        const approvalActivity: AIStreamEvent = { type: 'activity', activity: { runId: run.runId, type: 'action', message: 'O agente aguarda aprovação antes de continuar.', status: 'pending' } };
      ...
      const roundComplete: AIStreamEvent = { type: 'activity', activity: { runId: run.runId, type: 'complete', message: `Ciclo de ferramentas ${run.toolRounds} concluído.`, status: 'success' } };
