...
export class AgentRuntime {
  ...
  constructor(private readonly chatRuntime: ChatRuntime, private readonly tools: ToolRuntime, private readonly activity = new ActivityRuntime()) {}
  ...
  private async finishApproval(pending: PendingRun, approvalId: string): Promise<AgentRunResult> {
    ...
    if (pending.pendingApprovalIds.length) return { chatId: pending.chat.id, response: { content: '', model: pending.workingChat.model, providerId: pending.workingChat.providerId }, toolRounds: pending.toolRounds, pendingApprovalIds: [...pending.pendingApprovalIds], messages: [...pending.workingChat.messages] };
