...
const definitions: AIToolDefinition[] = [
  ...
  { name: 'run_command', description: 'Execute a local shell command from the active workspace. Use it for tests, builds, package managers, scripts, CLIs and operations that genuinely require a shell. Do not use it to create, edit, delete or rename workspace files when create_file, write_file, delete_file or rename_file can represent the operation, because those file tools provide diff review and stale-file protection. In read-only mode it is blocked. In every other permission mode it currently requires explicit user approval before the process starts because the shell does not yet have complete operating-system confinement. Sensitive direct mutations may be blocked entirely by the command safety policy.', parameters: { type: 'object', properties: { command: { type: 'string', description: 'Exact local shell command to execute.' } }, required: ['command'], additionalProperties: false }, requiresWriteAccess: false, requiresApproval: true },
