...
export class ApprovalRuntime {
  ...
  claim(id: string): ApprovalRequest {
    ...
    if (!approval || this.inFlight.has(id)) throw new Error('Aprovação não encontrada ou já processada.');
  ...
  resolve(id: string): ApprovalRequest {
    const approval = this.pending.get(id);
    if (!approval || this.inFlight.has(id) === false) throw new Error('Aprovação não encontrada.');
