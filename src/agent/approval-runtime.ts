...
export class ApprovalRuntime {
  ...
  setChatId(id: string, chatId: string): ApprovalRequest { const approval = this.pending.get(id); if (!approval) throw new Error('Aprovação não encontrada.'); const updated = { ...approval, chatId }; this.pending.set(id, updated); return updated; }
