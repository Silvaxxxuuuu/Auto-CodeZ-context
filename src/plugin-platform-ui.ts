...
function render(): void {
  ...
  panel.dataset.pluginPlatformRendered = 'true'; rendering = false;
}
...
export function initializePluginPlatformUi(): void {
  ...
  const observer = new MutationObserver(() => { if (rendering || !isPluginPanel()) return; const panel = document.querySelector<HTMLElement>('#nav-panel'); if (panel?.dataset.pluginPlatformRendered === 'true') return; selectedPluginId = null; render(); });
