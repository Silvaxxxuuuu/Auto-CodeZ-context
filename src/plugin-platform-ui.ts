...
if (bridge) {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (button.hasAttribute('data-plugin-save-permissions')) {
      const modal = button.closest<HTMLElement>('.plugin-permission-modal');
      const checked = modal ? [...modal.querySelectorAll<HTMLInputElement>('[data-plugin-permission-value]:checked')]
        .map((input) => input.getAttribute('data-plugin-permission-value'))
        .filter((permission): permission is string => Boolean(permission)) : [];
      const selected = plugin.requestedPermissions.filter((permission) => checked.includes(permission));
