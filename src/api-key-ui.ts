...
type ApiKeyApi = {
  ...
  saveApiKey: (input: { providerId: string; name: string; apiKey: string; model?: string; baseUrl?: string }) => Promise<unknown>;
  renameApiKey: (input: { keyId: string; name: string }) => Promise<ApiKeySummary>;
  setActiveApiKey: (keyId: string) => Promise<unknown>;
  removeApiKey: (keyId: string) => Promise<unknown>;
};
...
async function openApiKeyManager(): Promise<void> {
  ...
  const updateProviderFields = (): void => {
    const azure = providerInput.value === 'azure-openai';
    modelLabel.textContent = azure ? 'Deployment, opcional' : 'Modelo, opcional';
    modelInput.placeholder = azure ? 'Ex.: gpt-5.6-luna' : 'Modelo específico';
    baseUrlLabel.textContent = azure ? 'Endpoint Azure, obrigatório' : 'URL base, opcional';
    baseUrlInput.placeholder = azure ? 'https://seu-recurso.openai.azure.com' : 'https://...';
    baseUrlInput.required = azure;
  };
  const toggleForm = (open: boolean): void => { form.classList.toggle('open', open); if (open) nameInput.focus(); if (!open) form.reset(); };
  addButton.addEventListener('click', () => { if (!addButton.disabled) { toggleForm(true); updateProviderFields(); } }); cancelButton.addEventListener('click', () => toggleForm(false)); closeButton.addEventListener('click', closeApiKeyManager); providerInput.addEventListener('change', updateProviderFields); backdrop.addEventListener('click', (event) => { if (event.target === backdrop) closeApiKeyManager(); });
  form.addEventListener('submit', async (event) => { event.preventDefault(); const saveButton = queryRequired<HTMLButtonElement>(form, '.api-key-save'); const providerId = providerInput.value; const name = nameInput.value.trim(); const apiKey = keyInput.value.trim(); const model = modelInput.value.trim() || undefined; const baseUrl = baseUrlInput.value.trim() || undefined; if (!providerId || !name || !apiKey) return; if (providerId === 'azure-openai' && !baseUrl) { window.dispatchEvent(new CustomEvent('auto-codez-ui-error', { detail: 'Informe o endpoint do recurso Azure OpenAI.' })); baseUrlInput.focus(); return; } saveButton.disabled = true; saveButton.textContent = 'Validando...'; try { await bridge.saveApiKey({ providerId, name, apiKey, model, baseUrl }); toggleForm(false); await refresh(); } catch (error) { window.dispatchEvent(new CustomEvent('auto-codez-ui-error', { detail: error instanceof Error ? error.message : 'Não foi possível salvar a API key.' })); } finally { saveButton.disabled = false; saveButton.textContent = 'Salvar API Key'; } });
