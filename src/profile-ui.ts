...
async function renderProfile(): Promise<void> {
  ...
  content.innerHTML = `
    <section class="profile-card profile-identity-card">
      ...
      <span class="profile-status profile-status-neutral">Local-first</span>
    </section>

    <div class="profile-grid">
      <section class="profile-section">
        <div class="profile-section-heading"><div class="profile-section-icon">${icon('user-round')}</div><div><h2>Identidade local</h2><p>Use um nome para identificar este espaço de trabalho no Auto CodeZ.</p></div></div>
        <form class="profile-local-form" data-profile-local-form>
          ...
          <button class="profile-primary-button" type="submit">Salvar nome</button>
          <span class="profile-save-state" data-profile-save-state aria-live="polite"></span>
        </form>
      </section>

      <section class="profile-section">
        <div class="profile-section-heading"><div class="profile-section-icon">${icon('message')}</div><div><h2>Seu espaço de trabalho</h2><p>Números lidos do estado local atual, sem métricas fictícias.</p></div></div>
    ...
    <div class="profile-grid profile-grid-secondary">
      <section class="profile-section">
        <div class="profile-section-heading"><div class="profile-section-icon">${icon('sparkles')}</div><div><h2>Ambiente de IA</h2><p>Atalhos para gerenciar as fontes de inteligência usadas no produto.</p></div></div>
      ...
      <section class="profile-section">
        <div class="profile-section-heading"><div class="profile-section-icon">${icon('cpu')}</div><div><h2>Este dispositivo</h2><p>Informações úteis para IA Local e diagnóstico do ambiente.</p></div></div>
    ...
    <section class="profile-note">
      ${icon('shield')}
      <div><strong>Estado real, não conta simulada</strong><span>O Perfil não mostra mais login, sincronização ou métodos de acesso que ainda não existem. Privacidade, providers e IA Local levam às funções reais do aplicativo.</span></div>
...
document.addEventListener('submit', (event) => {
  ...
  if (state) state.textContent = 'Nome salvo neste dispositivo.';
}, true);
