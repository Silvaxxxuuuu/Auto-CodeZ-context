...
function visibleEvents(): LedgerEvent[] {
  const scoped = selectedScope ? events.filter((event) => scopeKey(event) === selectedScope) : events;
...
function currentHeader(): { title: string; subtitle: string; state: LedgerState; provider?: string; project?: string; run?: string } {
...
function renderEvent(event: LedgerEvent): string {
  return `<article class="mcp-event state-${escapeHtml(event.state)}" data-ledger-event="${escapeHtml(event.eventId)}">
  ...
  root.innerHTML = `
    ...
    <section class="mcp-mode-main">
      <header class="mcp-mode-header">
        ...
        <button class="mcp-refresh-button" type="button" data-mcp-refresh>Atualizar</button>
      </header>
      ...
      ${artifactIds.length ? `<section class="mcp-artifact-strip"><div class="mcp-section-label">Artifacts</div><div class="mcp-artifact-row">${artifactIds.map((id) => `<button class="mcp-artifact-chip" data-mcp-artifact="${escapeHtml(id)}"><span></span>${escapeHtml(id.slice(0, 14))}</button>`).join('')}</div></section>` : ''}
...
async function refresh(): Promise<void> {
  ...
  render();
  try {
    const [page, nextApprovals] = await Promise.all([
...
function installStyles(): void {
  ...
  style.textContent = `
    ...
    .mcp-mode-main{min-width:0;display:flex;flex-direction:column;overflow:hidden}.mcp-mode-header{height:69px;flex:none;padding:0 26px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #1b222c;background:#0a0e13}.mcp-mode-title-row{display:flex;align-items:center;gap:8px}.mcp-mode-title-row h1{margin:0;font-size:14px;font-weight:650}.mcp-header-state{padding:4px 7px;border:1px solid #283343;border-radius:999px;color:#8d99aa;font-size:8px}.mcp-mode-subtitle{margin-top:5px;color:#667283;font-size:8px}.mcp-refresh-button{height:30px;padding:0 10px;border:1px solid #293341;border-radius:7px;color:#9ca8b8;font-size:9px;background:#11161d}.mcp-refresh-button:hover{color:#eef2f7;border-color:#3b4859}
    .mcp-mode-toolbar{height:47px;flex:none;padding:0 24px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #171e27}.mcp-filter-group{display:flex;gap:4px}.mcp-filter{padding:6px 9px;border-radius:6px;color:#6d7888;font-size:9px}.mcp-filter:hover,.mcp-filter.active{background:#151b23;color:#dce3ec}.mcp-mode-count{font-size:8px;color:#5d6877}
