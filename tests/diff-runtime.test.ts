...
test('creates an exact line diff and summary', () => {
  ...
  const plan = runtime.createPlan([change], 'plan-1');
...
test('supports created, deleted, and renamed changes', () => {
  const runtime = new DiffRuntime();
  const changes = [
    runtime.create('new.ts', 'created', '', 'new file'),
    runtime.create('old.ts', 'deleted', 'old file', ''),
    runtime.create('new-name.ts', 'renamed', 'same content', 'same content', 'old-name.ts'),
  ];
  const plan = runtime.createPlan(changes, 'plan-2');
...
test('rejects invalid line counts supplied to summaries', () => {
  const runtime = new DiffRuntime();
  assert.throws(() => runtime.summarize([{
    path: 'src/a.ts',
    type: 'modified',
    before: 'a',
    after: 'b',
