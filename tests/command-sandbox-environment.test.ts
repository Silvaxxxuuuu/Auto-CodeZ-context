...
test('command sandbox isola home, config, temp e configuração Git do ambiente real do usuário', async () => {
  ...
  try {
    ...
    const sandboxDirectoryName = path.basename(temporaryRoot);
    const cwd = String(values.cwd);
    const gitCeiling = String(values.gitCeiling);

    assert.notEqual(home, fx.fakeHome);
    assert.match(sandboxDirectoryName, /^auto-codez-command-sandbox-/);
    ...
    assert.equal(path.basename(gitCeiling).toLowerCase(), 'workspace');
    assert.equal(path.basename(path.dirname(gitCeiling)), sandboxDirectoryName);
    assert.equal(path.basename(cwd).toLowerCase(), 'workspace');
    assert.equal(path.basename(path.dirname(cwd)), sandboxDirectoryName);
