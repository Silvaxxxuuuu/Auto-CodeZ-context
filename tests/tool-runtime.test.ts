...
test('write_file creates a diff-backed approval without modifying the file', async () => {
  ...
  try {
    ...
    const result = await fixture.runtime.execute('project-test', 'ask', call('call-2', 'write_file', { path: 'notes.txt', content: 'after' }));
...
test('approving a write_file executes the approved plan and returns the resulting diff', async () => {
  ...
  try {
    await fs.writeFile(path.join(fixture.root, 'notes.txt'), 'before');
    const pending = await fixture.runtime.execute('project-test', 'ask', call('call-3', 'write_file', { path: 'notes.txt', content: 'after' }));
...
test('approval refuses execution when the workspace changed after preview', async () => {
  ...
  try {
    await fs.writeFile(path.join(fixture.root, 'notes.txt'), 'before');
    const pending = await fixture.runtime.execute('project-test', 'ask', call('call-4', 'write_file', { path: 'notes.txt', content: 'after' }));
    assert.ok(pending.approvalId);
...
test('approving the same approval twice fails without executing twice', async () => {
  ...
  try {
    await fs.writeFile(path.join(fixture.root, 'notes.txt'), 'before');
    const pending = await fixture.runtime.execute('project-test', 'ask', call('call-5', 'write_file', { path: 'notes.txt', content: 'after' }));
    assert.ok(pending.approvalId);
...
test('invalid tool input is rejected before permission or execution', async () => {
  const fixture = await createToolRuntime();
  try {
    const result = await fixture.runtime.execute('project-test', 'unrestricted', call('call-6', 'read_file', { path: 'src/index.ts', unexpected: true }));
    assert.equal(result.ok, false);
...
test('read-only permission denies write tools', async () => {
  const fixture = await createToolRuntime();
  try {
    const result = await fixture.runtime.execute('project-test', 'read-only', call('call-7', 'write_file', { path: 'notes.txt', content: 'blocked' }));
    assert.equal(result.ok, false);
