...
function chat(permissionLevel: ChatRecord['permissionLevel'] = 'ask'): ChatRecord {
  return { id: 'chat-test', title: 'Agent Test', projectId: 'project-test', providerId: config.id, model: 'test-model', intelligence: 'normal', permissionLevel, messages: [{ role: 'user', content: 'Do the task.' }], createdAt: Date.now(), updatedAt: Date.now() };
}
...
test('stale approved changes remain pending and are never overwritten', async () => {
  const fixtureData = await fixture(fixtureDataResponses());
  try {
    const pending = await fixtureData.agent.run(config, chat(), undefined, 'ask');
    const approvalId = pending.pendingApprovalIds[0];
    const target = path.join(fixtureData.root, 'src', 'call-approval.ts');
    await fs.writeFile(target, 'changed outside agent');
    const result = await fixtureData.agent.resume(approvalId);
    assert.deepEqual(result.pendingApprovalIds, [approvalId]);
    assert.equal(result.response.content, '');
    assert.equal(await fs.readFile(target, 'utf8'), 'changed outside agent');
    assert.equal(fixtureData.agent.hasPendingForChat('chat-test'), true);
    const retry = await fixtureData.agent.resume(approvalId);
    assert.deepEqual(retry.pendingApprovalIds, [approvalId]);
  } finally { await fixtureData.cleanup(); }
});
